...
train_num = train[['AGE_YRS', 'NUMDAYS']]
train_cat = train.drop(['AGE_YRS', 'NUMDAYS'], axis=1)

test_num = test[['AGE_YRS', 'NUMDAYS']]
test_cat = test.drop(['AGE_YRS', 'NUMDAYS'], axis=1)
...
def scaling(s, e):
    ...
    if e == 'LabelEncoder':
        ...
        train_cat_encoded_5 = encoder.fit_transform(train_cat['ALLERGIES'])
        train_cat_encoded_6 = encoder.fit_transform(train_cat['VAX_MANU'])
        train_cat_encoded_7 = encoder.fit_transform(train_cat['SYMPTOM1'])
        ...
        train_cat_encoded_5 = pd.DataFrame(train_cat_encoded_5, columns=['ALLERGIES'])
        train_cat_encoded_6 = pd.DataFrame(train_cat_encoded_6, columns=['VAX_MANU'])
        train_cat_encoded_7 = pd.DataFrame(train_cat_encoded_7, columns=['SYMPTOM1'])
        ...
        train_cat_encoded = pd.concat([train_cat_encoded_0, train_cat_encoded_1,
                                       ...
                                       train_cat_encoded_6, train_cat_encoded_7], axis=1)
        # test_cat인코딩
        ...
        test_cat_encoded_5 = encoder.fit_transform(test_cat['ALLERGIES'])
        test_cat_encoded_6 = encoder.fit_transform(test_cat['VAX_MANU'])
        test_cat_encoded_7 = encoder.fit_transform(test_cat['SYMPTOM1'])
        ...
        test_cat_encoded_5 = pd.DataFrame(test_cat_encoded_5, columns=['ALLERGIES'])
        test_cat_encoded_6 = pd.DataFrame(test_cat_encoded_6, columns=['VAX_MANU'])
        test_cat_encoded_7 = pd.DataFrame(test_cat_encoded_7, columns=['SYMPTOM1'])
        # test_cat 데이터프레임 합치기
        test_cat_encoded = pd.concat([test_cat_encoded_0, test_cat_encoded_1,
                                      ...
                                      test_cat_encoded_6, test_cat_encoded_7], axis=1)
    # 스케일링
    train_num_scaled = scaler.fit_transform(train_num)
    test_num_scaled = scaler.fit_transform(test_num)
    # 스케일링 - 데이터 프레임으로 변환
    train_num_scaled = pd.DataFrame(train_num_scaled, columns=['AGE_YRS', 'NUMDAYS'])
    test_num_scaled = pd.DataFrame(test_num_scaled, columns=['AGE_YRS', 'NUMDAYS'])
